...
## Set print options => import my file => 'colorsStyles_printOptions.py'
import colorsStyles_displayOptions as cstyle  # Set print options
...
class Advanced_EDA():
    ...
    def _Df_describtion(self):
        print("\nData describtion:")
        print(self.df.describe(include='all').T)  # Includes categorical variable, Excluding Nan values.
        return
    ...
    # print all duplicated rows with Specific Value in wanted column.
    ...
    def get_SubDF_of_duplicates_for_specificVal_in_specificCol(self, col_name, checked_val, dropnav=False, \
        ...
        print("Sub-dataframes describtion:", SubDF_with_specific_value.describe().T)
    ...
    def Corr_with_plot_Find_and_remove(self, targetVal=None, corr_threshold=0.90, remove_negative=False, \
        ...
        if (targetVal is not None):
            df_corr = self.df.drop(targetVal,
                                   axis=1).corr()  # Compute pairwise correlation of columns, excluding Nan values
